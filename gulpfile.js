...
gulp.task('compile-pug', function(cb) {
  ...
  return gulp
    .src(['*.pug', '!_*.pug'], { cwd: baseDirs.source, since: gulp.lastRun('compile-pug', 100) })
    .pipe(remember('compile-pug'))
    .pipe(cached('compile-pug'))
...
gulp.task('stylelint', function(cb) {
  return gulp
    .src(options.scss.in, { since: gulp.lastRun('stylelint', 100) })
    .pipe(remember('stylelint'))
